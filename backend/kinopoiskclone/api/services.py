...
def serialize_value_list_films(qs, *columns):
    columns = 'id', 'name', 'year', 'type', 'image', *columns
    queryset = qs.distinct(*columns).values_list(
        *columns, 'genres__title', named=True
    )
    return queryset
